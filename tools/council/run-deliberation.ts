...
async function main():Promise<void>{
  ...
  const build=(role:string)=>JSON.stringify({role,instruction:"You are participating in a Council deliberation. Analyze the prompt and cast exactly one vote using the council CLI in this workspace.",required_steps:["Read session context.","Choose one of: approve, reject, abstain.",`Run: ~/openclaw/tools/council/council.sh vote --session ${sid} --voter ${role} --vote <approve|reject|abstain> --confidence <0-1> --reasoning '<brief rationale>' --model '<model>'`,"Return a concise summary with vote + confidence."],session:{id:sess.id,title:sess.title,type:sess.type,initiator:sess.initiator,participants:sess.participants,context:sess.context??{}}});
