...
describe("safe-memory-search", () => {
  it("prints help when no args", async () => {
    ...
    const mod = await importFresh("../../tools/memory/safe-memory-search.ts");
    expect(mod.runCli()).toBe(2);
  ...
  it("uses vector search when available", async () => {
    ...
    spawnSync.mockReturnValue({ status: 0, stdout: JSON.stringify([{ id: 1, text: "hello" }]), stderr: "" } as any);
  ...
  it("falls back to keyword search on vector failure", async () => {
    ...
    setArgv(["hamel"]);
    readJsonFile.mockReturnValue({ fallback_mode: false });
    spawnSync.mockReturnValue({ status: 1, stdout: "", stderr: "429 quota exceeded" } as any);
    ...
    fsMock.readFileSync.mockImplementation((p: string) => (p.endsWith("MEMORY.md") ? "- Hamel likes espresso" : "Hamel project notes"));
